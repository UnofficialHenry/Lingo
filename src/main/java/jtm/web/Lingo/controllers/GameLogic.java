...
public class GameLogic {
	...
	public String[] compare(String word, String word1) {
		...
		for(int i =0;i<word.length();i++) {
			if(Character.toLowerCase(word.charAt(i))==Character.toLowerCase(word1.charAt(i))) {
				color[i]="green";// letter exists and correct position - set letter color to GREEN
				continue;
			}
			for(int j=0;j<word1.length();j++) {
				if(Character.toLowerCase(word.charAt(i))==Character.toLowerCase(word1.charAt(j))) {
